"""
Модуль конфигурации (HYBRID STABLE).
1. UI_DATA_PATH -> Локальная папка проекта (UserBinnings, SolarHelioParams).
2. BASE_DATA_PATH -> Внешний диск (для тяжелых файлов RBflux).
"""
import os
import numpy as np
...
# 1. ЛОКАЛЬНАЯ ПАПКА (Внутри проекта) - Для Метаданных, Дней, Погоды
...
# === КЛЮЧЕВЫЕ ФАЙЛЫ (Берем строго из локальной папки!) ===
...
def _load_mat_file(path):
    ...
    try: 
        # squeeze_me=True упрощает массивы, struct_as_record=False делает объекты
        # НО иногда возвращает dict, поэтому в коде будем использовать get_val
...
def load_metadata_full():
    ...
    # Для совместимости, пустые данные о днях (Dialog загрузит их сам из локальных файлов)

...
Исправлена ошибка ValueError: ambiguous truth value.
Полная синхронизация с DrawSpectra.m и CheckProtocol.
"""
import os
import numpy as np
import warnings
...
def _load_mat_file(file_path):
    ...
    try:
        # squeeze_me=True важен для соответствия индексации MATLAB
...
def _get_spectra_data(app_state, ax_index):
    """Реализация логики из DrawSpectra.m."""
    print(f"\n[PROCESSING] -> Старт построения спектра...")
    ...
    accumulated_y = []
    accumulated_y_err = []
    FUNITS = 1e7 # Коэффициент из DrawSpectra.m

    for fpath in files:
        mat = _load_mat_file(fpath)
        if mat is None: continue
        
        # БЕЗОПАСНЫЙ поиск переменных (исправление ValueError)
        j_data = None
        ...
        if j_data is None:
            print(f"    [WARN] Данные J/Jday не найдены в {os.path.basename(fpath)}")
            continue

        try:
            # MATLAB: Jday(L, E, P). В Python (3, 6, 16) соответствует этому порядку
    ...
    # Возврат данных для matplotlib_widget
    return [{
        "ax_index": ax_index,
        "plot_type": "errorbar",
        "x": x_centers[mask],
...
def get_plot_data(app_state, ax_index=0):
    # plot_kind 0: Energy, 1: Rigidity (согласно CheckProtocol)

"""
Порт ShowDays (SMART PATH EDITION).
Восстановлено красивое окно с календарем, легендой и космической погодой.
Добавлен умный поиск файлов внутри dirflux_newStructure.
"""
import os
import numpy as np
...
class DaysDialog(QDialog):
    ...
    def __init__(self, app_state: ApplicationState, parent=None):
        ...
        self.setWindowTitle("Mission Timeline & Info (Smart Edition)")
        ...
        # Загрузка данных (С УМНЫМ ПОИСКОМ)
    ...
    # === УМНЫЙ ПОИСК ФАЙЛОВ ===
    def _find_smart_path(self, subfolder, filename):
        """Ищет файл в BASE_DATA_PATH и внутри dirflux_newStructure"""
        base = config.BASE_DATA_PATH
        
        # 1. Прямой путь (data/UserBinnings/file.mat)
        p1 = os.path.join(base, subfolder, filename)
        if os.path.exists(p1): return p1
        
        # 2. Внутри dirflux (data/dirflux_newStructure/UserBinnings/file.mat)
        p2 = os.path.join(base, 'dirflux_newStructure', subfolder, filename)
    ...
    def load_data(self):
        """Загружает Tbinning_day.mat для раскраски."""
        path = self._find_smart_path('UserBinnings', 'Tbinning_day.mat')
        
        if not path:
            print("Warning: Tbinning_day.mat not found! Calendar will be uncolored.")
            # Fallback: Если нет файла качества, используем список из config (MagParam)
            # и красим все в зеленый (как временное решение)
            if hasattr(config, 'FILE_INFO') and 'RunDays' in config.FILE_INFO:
                 for d in config.FILE_INFO['RunDays']:
                     self.day_quality_map[int(d)] = 1 # Good
            return
    ...
    def load_solar_data(self):
        try:
            path_b = self._find_smart_path('SolarHelioParams', 'Bartels.mat')
            ...
            path_c = self._find_smart_path('SolarHelioParams', 'Carrington.mat')
    ...
    def load_mag_data(self):
        path = self._find_smart_path('SolarHelioParams', 'MagParam2.mat')
        mat = config._load_mat_file(path)
        if not mat: return
        ...
        try:
            ...
            # Сохраняем
            # Если day_quality_map пуст (нет Tbinning), заполняем его из MagParam
            ...
            for i, day in enumerate(unique_days):
                ...
                self.mag_data_map[d_int] = {
                    'Kp': kp_max[i], 'Dst': dst_min[i], 'F10.7': f10_mean[i]
                }
        except Exception as e:
    ...
    def on_cell_clicked(self, row, col):
        ...
        if item:
            ...
            if self.bartels_data:
                ...
                if idx >= 0 and idx < len(self.bartels_data['bn']):
            ...
            if pam_day in self.mag_data_map:
                ...
                html += "<i>No weather data available</i>"
